...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- Additional Cached Elements for Performance ---
    const generalMemoFieldField = document.getElementById('general-memo');
    ...
    // --- Cached DOM Collections for Performance ---
    let cachedCheckboxes = null;
    let cachedMemoInputs = null;
    let cachedMemoTextareas = null;
    let cachedUrlInputs = null;
    ...
    function renderNotesTable(allMonthData) {
        ...
        updateDOMCache(); // キャッシュを更新
        addNotesEventListeners();

        // 月次メモ欄にURL自動リンク化を適用
        setTimeout(() => {
            if (!cachedMemoTextareas) {
                cachedMemoTextareas = notesTableBody.querySelectorAll('textarea[data-field="memo"]');
            }
            cachedMemoTextareas.forEach(textarea => {
    ...
    // --- DOM Cache Update Function ---
    function updateDOMCache() {
        // DOM変更後にキャッシュを無効化・更新
        cachedCheckboxes = null;
        cachedMemoInputs = null;
        cachedMemoTextareas = null;
        cachedUrlInputs = null;
    }

    function getCachedCheckboxes() {
        if (!cachedCheckboxes) {
            cachedCheckboxes = detailsTableBody.querySelectorAll('input[type="checkbox"]');
        }
        return cachedCheckboxes;
    }

    function getCachedMemoInputs() {
        if (!cachedMemoInputs) {
            cachedMemoInputs = detailsTableBody.querySelectorAll('.checkbox-memo-input');
        }
        return cachedMemoInputs;
    }

    function getCachedMemoTextareas() {
        if (!cachedMemoTextareas) {
            cachedMemoTextareas = notesTableBody.querySelectorAll('textarea[data-field="memo"]');
        }
        return cachedMemoTextareas;
    }

    function getCachedUrlInputs() {
        if (!cachedUrlInputs) {
            cachedUrlInputs = notesTableBody.querySelectorAll('input[data-field="url"]');
        }
        return cachedUrlInputs;
    }

    // --- Event Listeners ---
    function addCheckboxEventListeners() {
        getCachedCheckboxes().forEach(checkbox => {
        ...
        // メモ入力欄のイベントリスナーを追加
        getCachedMemoInputs().forEach(memoInput => {
        ...
        if (generalMemoFieldField) {
            generalMemoFieldField.addEventListener('input', (e) => {
                setUnsavedChanges(true);
            });
        }
    }
    ...
    function addNotesEventListeners() {
        ...
        // 初期表示のために全URL入力をチェック
        getCachedUrlInputs().forEach(updateUrlInputStyle);
    }
    ...
    async function saveAllChanges() {
        ...
        try {
            ...
            getCachedCheckboxes().forEach(cb => {
            ...
            // メモデータの保存ロジックを修正
            const memoInputs = getCachedMemoInputs();
            ...
            [...getCachedUrlInputs(), ...getCachedMemoTextareas()].forEach(input => {
    ...
    async function saveOverallMemo() {
        if (generalMemoFieldField && clientId) {
            const memoValue = generalMemoFieldField.value.trim();
            await SupabaseAPI.updateClient(clientId, { 
    ...
    function loadOverallMemo() {
        if (generalMemoFieldField && clientDetails) {
            const memoValue = clientDetails.overall_memo || '';
            generalMemoFieldField.value = memoValue;
        }
    }
    ...
    function initializeAutoLinkify() {
        ...
        setTimeout(() => {
            const memoTextareas = getCachedMemoTextareas();
    ...
    function updateDetailsToggleButtonText(mode) {
        const toggleButton = detailsTable.querySelector('#details-table-mode-toggle-btn');
